...
const ILI_BILLING_FROM_VARIANTS = ['billing_from', 'BILLING_FROM', 'invoice_start_date', 'INVOICE_START_DATE', 'SERVICE_START_DATE']
...
const QLI_TERM_VARIANTS = ['renewal term', 'renewal_term', 'RENEWAL_TERM']
 ...
 * If (today > service_start_date + initial_term) AND (today < service_start_date + initial_term + term): CUP = Unit Price * (1+Increment)
 * If today > service_start_date + initial_term + term: CUP = Unit Price * (1+Increment) * (1+Increment)^num_completed_terms
 */
function getCUP(ili, quoteItem, today) {
  ...
  const initialTermMonths = getNumeric(ili, ILI_FIRST_PRICE_INC_VARIANTS)
  ...
  const initialTermIncrement = increment
...
export function runValidation(baseData, quoteData, options = {}) {
  ...
  for (let i = 0; i < (baseData || []).length; i++) {
    ...
    const baseResult = {
      ...
      billing_from: getValue(ili, ILI_BILLING_FROM_VARIANTS),
      billing_till: getValue(ili, ILI_BILLING_TILL_VARIANTS),
      pf: getPF(ili),
      validation_result: '',
      remarks: ''
    }
